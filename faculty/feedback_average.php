...
<select name="faculty" class="form-control" id="sl">
	...
	$sql=mysqli_query($conn,"select distinct staff_id,staffname from staff where department='$_SESSION[department]' order by staff_id");
...
if (isset($select))  {
	...
	echo "<h2>Subjects Handled By <b>".$r['staffname']."</b> for <b>".$_SESSION['department']."</b> Academic Year <b>".$chac."</b></h2>";

	$sql1=mysqli_query($conn,"select * from staff where staff_id='$faculty' and department='$_SESSION[department]' and acyear='$chac' order by sem");
...
if(isset($sub))
{
    $sql1=mysqli_query($conn,"select * from staff where staff_id='$_SESSION[faculty]' ");

	$r1=mysqli_fetch_array($sql1);
$dp=strtolower($_SESSION['department']);
$tb="feedback-".$dp."-"."$_SESSION[acyear]";
...
$stcount=mysqli_query($conn,"select * from user where acyear='$_SESSION[acyear]' and programme='$_SESSION[department]' ");
...
echo "<br><br><span id='avg'>Total Average for 100 : <span id='av'>".$avg. "% </span>  </span>" ;
echo "<span id='avg' style='margin-left:10%;'>Total Average for 5 : <span id='av'>".$avg1. "% </span>  </span>" ;
...
<div class="bar">
    ...
    <form method='POST' action='generate_report.php' id="reportform" target="_blank">
<button class='btn btn-warning' name='reportbtn' id='reportbtn'> <i class='fa fa-download' style='color:yellow;'></i> Download Report</button>
</form>
 <br>
 <br>
 <br>
