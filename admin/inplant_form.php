    ...
    <tr>
        ...
        if(isset($_POST['filter'])){
            if(($date == "" )&&($chsem =="")&&($chyear=="")&&($tdate == "")){
                ...
                $sql=mysqli_query($conn,"select * from inplant");
            }
            elseif(($date != "" )&&($chsem =="")&&($chyear !=="")&&($tdate != "")){
                $sql=mysqli_query($conn,"select * from inplant where fromdate='$date'");
                $search="<h2>Search Results for Date from <b>".$date."</b> to <b>".$tdate."</b> and Year <b>".$chyear."</b> </h2>";
                 }
            elseif(($date == "" )&&($chsem !="")&&($chyear !="")&&($tdate == "")){
                 $sql=mysqli_query($conn,"select * from inplant where sem='$chsem'");
            ...
            elseif(($date == "" )&&($chsem =="")&&($chyear !="")&&($tdate == "")){
                 $sql=mysqli_query($conn,"select * from inplant where year='$chyear'");
                 $search="<h2>Search Results for Year <b>".$chyear."</b> </h2>";
                 }
        
            else {
                $sql=mysqli_query($conn,"select * from inplant where fromdate='$date' and todate='$tdate'");
