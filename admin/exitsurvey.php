    ...
    <tr>
        ...
        if(isset($_POST['filter'])){
            if(($chac == "" )&&($chdept =="")){
                ...
                $sql=mysqli_query($conn,"select * from exitsurvey order by regno asc");
            }
            elseif(($chac !== "" )&&($chdept =="")){
                $sql=mysqli_query($conn,"select * from exitsurvey where academic='$chac' order by regno asc");
                $search="<h2>Search Results for Period of Study <b>".$chac."</b> </h2>";
                 }
            elseif(($chac == "" )&&($chdept !="")){
                 $sql=mysqli_query($conn,"select * from exitsurvey where department='$chdept' order by regno asc");
                 $search="<h2>Search Results for Department <b>".$chdept."</b> </h2>";
                 }
            else {
                $sql=mysqli_query($conn,"select * from exitsurvey where academic='$chac' and department='$chdept' order by regno asc");
                $search="<h2>Search Results for Period of Study <b>".$chac."</b> and Department <b>".$chdept."</b> </h2>";
                }
        }else{
            $sql=mysqli_query($conn,"select * from exitsurvey order by regno asc");
        }
