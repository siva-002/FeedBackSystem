...
$q=mysqli_query($conn,"select  * from user order by regno asc");
...
<h3 style="text-decoration:underline;">Sort by</h3>
...
<div id="main">
<table class="table table-bordered table-hover" id="table" style=' table-layout:fixed; width:1000px;'>
   ...
   <th>Feedback Not Given  </th>
    ...
    <?php
        if(isset($_POST['filter'])){
            ...
            elseif(($chdept != "" )&&($chac =="")){
                $q=mysqli_query($conn,"select * from user where programme='$chdept' order by regno asc");
                $search="<h3>Search results for department <b>".$chdept."</b></h3>";
            }
            elseif(($chdept == "" )&&($chac !="")){
                $q=mysqli_query($conn,"select * from user where acyear='$chac' order by regno asc");
                $search="<h3>Search results for Academic Year <b>".$chac."</b></h3>";
            }
            else{
                $q=mysqli_query($conn,"select * from user where programme='$chdept' and acyear='$chac' order by regno asc");
                $search="<h3>Search results for department <b>".$chdept."</b> and Academic Year <b>".$chac."<b></h3>";
            }
        }
    ...
    $fbtable='feedback-'.$dept.'-'.$res['acyear'];
     ...
     $lt=substr($res['regno'],8,2);
     $tr=substr($res['regno'],8,2);
     if(($lt == "3") or ($tr == "7")){
        ...
        /*if(($res['acyear']=="2019-2023") && ($res['programme'=="CSE"]))
     ...
     $qry1=mysqli_query($conn,"select * from `{$fbtable}` where department='$res[programme]' and regno='$res[regno]' order by semester ASC");
        ...
        if(($lt=="3")or($tr=="7")){
                   $qry3=mysqli_query($conn,"select staff.coursecode , staff.sem from staff where staff.sem NOT IN ('i','ii') and staff.department='$res[programme]' and staff.acyear='$res[acyear]' and staff.coursecode not in 
