    ...
    <tr>
        ...
        if(isset($_POST['filter'])){
            if(($date == "" )&&($chsem =="")&&($chyear=="")){
                ...
                $sql=mysqli_query($conn,"select * from industry");
            }
            elseif(($date != "" )&&($chsem =="")&&($chyear !="")){
                $sql=mysqli_query($conn,"select * from industry where Year='$chyear'");
                $search="<h2>Search Results for Date <b>".$date."</b> and Year <b>".$chyear."</b> </h2>";
                 }
            elseif(($date == "" )&&($chsem !="")&&($chyear !="")){
                 $sql=mysqli_query($conn,"select * from industry where Semester='$chsem'");
            ...
            elseif(($date != "" )&&($chsem =="")&&($chyear =="")){
                 $sql=mysqli_query($conn,"select * from industry where Date='$date'");
                 $search="<h2>Search Results for Date <b>".$date."</b> </h2>";
                 }

            else {
                $sql=mysqli_query($conn,"select * from industry where Date='$date' and Semester='$chsem'");
