...
elseif(isset($_POST['cse'])){
	...
	if(isset($_POST['search'])){
		$skey=$_POST['search'];
		$que=mysqli_query($conn,"select * from user where regno like '$skey%' order by regno");
	...
	elseif($value =="ALL")
		$que=mysqli_query($conn,"select * from user order by regno asc");
	else
		$que=mysqli_query($conn,"select * from user where programme='$value' and acyear='$chac' order by regno asc");
...
?>
	...
	echo "<h2>".$value."&nbsp;<b>". @$chac ."</b> STUDENT DETAILS <b>( $count )</b></h2>";
	}
